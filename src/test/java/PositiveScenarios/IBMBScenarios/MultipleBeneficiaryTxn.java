...
public class MultipleBeneficiaryTxn extends PageObject {
    ...
    public void iLoginAsSupervisorToActivateMultipleBeneficiaryRule() {
    ...
    public void theMultipleBeneficiaryRuleIsOn() {
    ...
    public void checkTheMultipleBeneficiaryRuleIsActiveOrNot() {
    ...
    public void iPostTransactionForMultipleBeneficiary() {
        IBMBBeneficiary ibmbBeneficiary = new IBMBBeneficiary(currentDate + " " + currentTime,"10.212.134.38","Add",currentDate + " " + currentTime,0,0,"INR",txnId,"000007","MBX","Savings Account","000004","HDFC22142","HDFC","ANDHERI","KRUPA DEVI","KRUPA","213354657","345345345"," 214241 ","","","","","");
        HashMap<String,Object> multipleBeneficiary = TxnBody.BeneficiaryTransactionBody(ibmbBeneficiary);
        given().contentType(ContentType.JSON).
                with().body(multipleBeneficiary).
                when().post(postUrl).
                then().assertThat().statusCode(200);

        IBMBBeneficiary ibmbBeneficiary1 = new IBMBBeneficiary(currentDate + " " + currentTime,"10.212.134.38","Add",currentDate + " " + currentTime,0,0,"INR",txnId1,"000007","MBX","Savings Account","000005","HDFC22142","HDFC","DADAR","CHANDANI DEVI","CHANDANI","213454657","345345345"," 214242 ","","","","","");
        HashMap<String,Object> multipleBeneficiary1 = TxnBody.BeneficiaryTransactionBody(ibmbBeneficiary1);
        given().contentType(ContentType.JSON).
                with().body(multipleBeneficiary1).
                when().post(postUrl).
                then().assertThat().statusCode(200);

        IBMBBeneficiary ibmbBeneficiary2 = new IBMBBeneficiary(currentDate + " " + currentTime,"10.212.134.38","Add",currentDate + " " + currentTime,0,0,"INR",txnId2,"000007","MBX","Savings Account","000006","HDFC22142","HDFC","BANDRA","PUSPA DEVI","PUSPA","213554657","345345345"," 214243 ","","","","","");
        HashMap<String,Object> multipleBeneficiary2 = TxnBody.BeneficiaryTransactionBody(ibmbBeneficiary2);
        given().contentType(ContentType.JSON).
                with().body(multipleBeneficiary2).
                when().post(postUrl).
                then().assertThat().statusCode(200);
    }

    @And("If i get action as {string} for MultipleBeneficiary")
    public void ifIGetActionAsForMultipleBeneficiary(String arg0) {
        IBMBTxn ibmbTxn =  new IBMBTxn(txnId3,currentDate + " " + currentTime,"000007","700000",20000.00,currentDate + " " + currentTime,"Test","7000583101","INR","500000","FRM NEFT TEST","","111.91.231.192","NEFT","CITI0000003","","jashantiwari25@gmail.com", 28.613939,77.209023,"INR","A|CITIBANK NA","","","","mbx");
        HashMap<String, Object> multipleBeneficiary3 = TxnBody.IBMBTransactionBody(ibmbTxn);
        given().contentType(ContentType.JSON).
                with().body(multipleBeneficiary3).
                when().post(postUrl).
                then().assertThat().body("action", Is.is("stop")).statusCode(200);
    }

    @Then("I turn off the MultipleBeneficiary rule")
    public void iTurnOffTheMultipleBeneficiaryRule() {
        given().cookies(authorization, token).
                when().put(RuleOperations.ecomActivateRule(ruleId)).
                then().statusCode(200);

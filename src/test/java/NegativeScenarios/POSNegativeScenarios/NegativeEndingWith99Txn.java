...
public class NegativeEndingWith99Txn extends PageObject {
    ...
    @Given("I login as supervisor to activate EndingWith{int}Txn rule for negative scenarios")
    public void iLoginAsSupervisorToActivateEndingWithTxnRuleForNegativeScenarios(int arg0) {
    ...
    @And("The EndingWith{int}Txn rule is on for negative scenarios")
    public void theEndingWithTxnRuleIsOnForNegativeScenarios(int arg0) {
    ...
    @And("Check the EndingWith{int}Txn rule is active or not for negative scenarios")
    public void checkTheEndingWithTxnRuleIsActiveOrNotForNegativeScenarios(int arg0) {
    ...
    @When("I post transactions ending with {int} for negative scenarios")
    public void iPostTransactionsEndingWithForNegativeScenarios(int arg0) {
        POSTxn posTxn = new POSTxn("1200", "6080140888812222", "001000", 9900, currentDate + " " + currentTime, txnId, currentDate + " " + currentTime, "356", "71020171000C", "200", "8520", "810697", "000000415430", "235678", "RB0058080000000", "Pune", "356", "300000", "000003");
...
then().assertThat().body("responseCode", Is.is("000")).statusCode(200);
    ...
    @And("If i get response code as {int} for EndingWith{int}Txn for negative scenarios")
    public void ifIGetResponseCodeAsForEndingWithTxnForNegativeScenarios(int arg0, int arg1) {
        POSTxn posTxn1 = new POSTxn("1200", "6080140888812222", "001000", 9800, currentDate + " " + currentTime, txnId1, currentDate + " " + currentTime, "356", "71020171000C", "200", "8520", "810697", "000000415430", "235678", "RB0058080000000", "Pune", "356", "300000", "000003");
...
then().assertThat().body("responseCode", Is.is("000")).statusCode(200);
    ...
    @Then("I turn off the EndingWith{int}Txn rule for negative scenarios")
    public void iTurnOffTheEndingWithTxnRuleForNegativeScenarios(int arg0) {
        given().cookie(authorization, token).
                when().put(RuleOperations.posActivateRule(ruleId)).
                then().statusCode(200);
